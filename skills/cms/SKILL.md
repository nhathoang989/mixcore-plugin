...
1. Read `plugins/mixcore/skills/guide/server-config.md`.
   - If it exists and contains a server name → set `MCP_PREFIX = mcp__{server-name}__` (e.g. `mcp__mixcore-cloud__`).
   - If the file does not exist → run **Step 0** from the `mixcore:guide` skill to detect and persist the server, then return here.
2. Replace every `mcp__mixcore__` reference in this skill with `{MCP_PREFIX}`.

> This step is skipped automatically when `mixcore:cms` is invoked via the `mixcore:guide` router (which already resolved `{MCP_PREFIX}` in its own Step 0).
...
| **AI chat widgets** — SiteKnowledgeHub integration, SignalR frontend, drawer pattern | Use `mixcore:ai` skill |

For AI chat widget tasks (floating chat, SiteKnowledgeHub, SignalR-connected widgets), invoke the **`mixcore:ai`** skill instead of or alongside this one.
...
**Exception — wiki docs**: Files under `wwwroot/mixcontent/documents/wiki/` must be managed with **`mixcore:rag`** tools (`generate_document`, `read_document`, `list_documents`, `delete_document`) — never `WriteTextFile`/`ReadTextFile`. Those bypass the RAG index and leave the in-app AI with stale content.
...
Before executing any CMS task, **search the wiki knowledge base** to load site context. Load the **`mixcore:rag`** skill, then:
...
| **Wiki / RAG** | `Search`, `GenerateDocument`, `ReadDocument`, `ListDocuments`, `DeleteDocument`, `ReloadWiki` | Wiki knowledge base — load **`mixcore:rag`** skill |
...
> build task. Use `mixcore:rag` only when you want to hand-author a standalone wiki doc.
