...
| **AI chat widget** on a CMS page — floating/drawer chat, SiteWikiHub SignalR wiring, streaming, login/token, auth-failure handling | `mixcore:mix-mcp-ai` |
...
- **Build then prove it works** → `mixcore:mix-mcp-build-site` (build) → `mixcore:mix-verify-site` (drive the pages + cross-check the data)
...
- **"Run X on a schedule / every N minutes" (single action)** → `mixcore:mix-mcp-schedule` (cron job → Webhook or QueuePublish)
- **"When X happens, do A then B then C" (event/webhook trigger, multi-step)** → `mixcore:mix-mcp-flows`

> **Scheduler vs Flows:** time-based + one action → `mixcore:mix-mcp-schedule`. Webhook/manual/queue trigger or multi-step pipeline → `mixcore:mix-mcp-flows`. A Flows `Schedule` trigger overlaps for simple recurring single actions — prefer `mixcore:mix-mcp-schedule` when there are no extra steps.
...
Full `/mcp` catalog grouped by the leaf skill that owns each family — route the task, then use the owning skill's tool section (exact `tools/list` names).

- **Templates + content — `mixcore:mix-mcp-cms`** — `create_template`/`validate_template`/`update_template`/`list_templates`, `create_page_content`/`update_page_content`/`list_page_contents`, `create_module_content`/`list_module_contents`, `create_post_content`, page↔module/post associations, themes, culture/language (multilingual)
- **MixDB — `mixcore:mix-mcp-db`** — `create_mix_db_table`, `create_mix_db_table_from_prompt`, `create_column`, `create_relationship`, `migrate_mix_db_table`, `query_table`, `query_rows`, `create_row`, `update_row`, `smart_query`, `execute_query`
- **Wiki / RAG — `mixcore:mix-mcp-rag`** — `search`, `search_backend_knowledge`, `generate_document`, `read_document`, `list_documents`, `delete_document`, `reload_wiki`
- **Flows — `mixcore:mix-mcp-flows`** — `create_workflow`, `trigger_workflow`, `list_workflows`, `get_run_history`, `cancel_run`
- **Scheduler — `mixcore:mix-mcp-schedule`** — `create_scheduled_job`, `run_scheduled_job_now`, `list_scheduled_jobs`, `toggle_scheduled_job`, `list_job_runs`
- **SPA embed — `mixcore:mix-mcp-spa`** — `write_text_file`, `upload_from_url`, `create_template`+`create_page_content` with `layoutId=null`
- **Site build — `mixcore:mix-mcp-build-site`** — orchestrates the above across 7 phases
- **AI generation** — `generate`, `generate_text`, `generate_image`, `generate_audio`, `generate_video`, `generate_embeddings`, `analyze_image`, `get_generation_job`
- **Storage** — `upload_from_url`, `get_presigned_upload_url`
- **AI plans** — `list_ai_plans`, `get_ai_plan`, `cancel_ai_plan`, `re_run_ai_plan`, `resume_ai_plan`, `retry_ai_plan`, `continue_ai_plan`, `delete_ai_plan`
- **Notifications** — `create_notification`, `list_notifications`, `get_unread_count`, `mark_read`, `mark_all_read`
- **Service discovery / utility** — `discover_service`, `list_services`, `fetch`, `echo`
<!-- /mcp-tools:auto -->
