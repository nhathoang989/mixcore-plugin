---
name: ai
description: Implement AI chat backend functions for Mixcore CMS — SignalR hub connection, AskAI invocation, streaming event handlers, auth failure detection, token management. UI layout and CSS are handled by the mixcore:cms skill.
...
- **Not covered here:** Drawer/overlay HTML structure, CSS layout, trigger button HTML, header button HTML/CSS. For UI implementation invoke the **`mixcore:cms`** skill (it owns Razor templates and CSS-in-content-field).
...
CSS for the AI bubble (`.mix-b`) belongs in the template content field — implement via the mixcore:cms skill.
...
- [ ] UI structure (drawer HTML, overlay, trigger button) implemented via the mixcore:cms skill
...
- Never implement drawer CSS, overlay HTML, or trigger button HTML in this skill — delegate to mixcore:cms
