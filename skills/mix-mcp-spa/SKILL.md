...
**When NOT to use it** — if the page should be **rendered by Razor** with Mixcore data (Model.Content, MixDB queries, modules, posts), use [mixcore:mix-mcp-cms](../mixcore:mix-mcp-cms/SKILL.md) instead. This skill is for fully client-rendered apps that ship their own runtime.
...
1. Invoke the `mixcore:mix-mcp-cms` skill and load `mixcore:mix-mcp-db`
...
## Dynamic data & forms — consult `mixcore:mix-mcp-cms` BEFORE writing code

**Before generating any SPA code that loads data or renders a form, invoke the `mixcore:mix-mcp-cms` skill** and inspect the live Mixcore site state (MixDB tables, posts, pages, modules). Data that lives in the CMS must be fetched at runtime — never hardcoded.
...
| List of products, blog posts, team members | ❌ No | Load `mixcore:mix-mcp-cms` → find/create MixDB table → fetch via API |
| Contact / signup / survey form | ❌ No | Load `mixcore:mix-mcp-cms` → identify form module or MixDB table for submissions |
| Navigation items | ❌ No | Load `mixcore:mix-mcp-cms` → `get_page_navigation_tree` |
| Images whose URLs are known build-time constants | ✅ Fine | Inline the URL |
| Images managed by editors (changeable) | ❌ No | Load `mixcore:mix-mcp-cms` → store in MixDB row with image column |

### Data-loading pattern

After loading `mixcore:mix-mcp-cms` and confirming the MixDB table name (e.g. `products`), fetch at runtime using the Mixcore REST API:
...
Before building a form, load `mixcore:mix-mcp-cms` and call `get_modules_by_type` or `list_module_contents` to check for an existing form module. If one exists, submit to its endpoint. If not, create a MixDB table for submissions first (via `mixcore:mix-mcp-cms` → `mixcore:mix-mcp-db`), then POST rows:
...
User asks for data-loading or form in SPA
  → invoke mixcore:mix-mcp-cms skill
...
- ❌ Never hardcode data that belongs in the CMS (product lists, blog posts, form submissions, nav items) — load `mixcore:mix-mcp-cms` first, confirm the MixDB table/module, then fetch via the Mixcore REST API at runtime
- ❌ Never write a form `POST` or data `GET` against `/api/...` without first using `mixcore:mix-mcp-cms` to verify the endpoint and column schema exist
...
Canonical `/mcp` tools this skill drives when embedding a built SPA (exact `tools/list` names).

- **Deploy static assets** — `write_text_file`, `list_text_files`, `read_text_file`, `delete_text_file`, `upload_from_url`, `get_presigned_upload_url`
- **Page install (`layoutId=null`)** — `create_template`, `update_template`, `get_template`, `create_page_content`, `update_page_content`, `get_page_content_by_seo_name`
<!-- /mcp-tools:auto -->
