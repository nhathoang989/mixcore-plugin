---
name: guide
description: Entry point for CONTENT tasks in Mixcore CMS — routes to the mixcore:* skill that uses MCP tools to CRUD content (templates, pages, modules, posts, MixDB tables/rows, site wiki, AI chat widget). Content only — never edits repo source code. Trigger on "mixcore", "mixcore:*", or any templates / MixDB / pages / modules / posts / wiki / website-content task. For source-code feature work use mixcore:dev; to keep skill↔system-prompt docs in sync use mixcore:docs-sync.
argument-hint: "<describe your task — auto-routes to the right mixcore:* skill>"
---

# Mixcore Skill Router

> **Scope:** this router covers **content CRUD via MCP tools only** — templates, pages, modules, posts, MixDB schema/rows, the site wiki, and the AI chat widget. It never edits repo source code. Sibling routers: **`mixcore:dev`** (AI edits C#/Blazor source to implement features) · **`mixcore:docs-sync`** (CRUD developer docs, keep skills ↔ system-prompts consistent).

When invoked with a task (e.g. `/mixcore:guide create a contact form` or `/mixcore:guide add a price column to products`), you **must**:
...
Use the `Read` tool to read `plugins/mixcore/skills/guide/server-config.md`.
...
Write the chosen server name (just the key, e.g. `mixcore-bk`) to `plugins/mixcore/skills/guide/server-config.md`:
...
> **Reset:** The user can delete `plugins/mixcore/skills/guide/server-config.md` at any time to be asked again on the next invocation.

---

## Step 1 — Search the Site Wiki First (via mixcore:rag)
...
> For all wiki document operations (search, read, create, list, delete) load the **`mixcore:rag`** skill.
...
**When you need to read, create, or delete a specific wiki doc** — invoke **`mixcore:rag`** and use `{MCP_PREFIX}read_document`, `{MCP_PREFIX}generate_document`, or `{MCP_PREFIX}delete_document`. Never use raw `read_text_file`/`write_text_file` for wiki files.
...
| Task signal | Skill |
|---|---|
| `.cshtml` template, CMS page/module/post, MixDB-driven content rendering | `mixcore:cms` |
| MixDB table/column/relationship schema, seed rows (no rendering) | `mixcore:db` |
| Embed a **built SPA** (Vite/React/Vue/Svelte/Next-static) as a Mixcore page with `layoutId=null` — **only when the user explicitly requests `mixcore:spa` or says they have a `dist/` folder to deploy** | `mixcore:spa` |
| Complete website from a brief — phased plan, schema + templates + pages; **also the default for any React/Vue/Svelte/frontend page/landing page request unless `mixcore:spa` is explicitly requested** | `mixcore:build-site` |
| **AI chat widget** on a CMS page — floating/drawer chat, SiteKnowledgeHub SignalR wiring, streaming, login/token, auth-failure handling | `mixcore:ai` |
| **Wiki / knowledge base / RAG** — search the site wiki, create/read/list/delete wiki docs, manage the RAG index | `mixcore:rag` |
| Coding / backend / Blazor / .NET module work | → route to `mixcore:dev` skill instead |

---

## Combination Patterns (first skill = invoke now)

- **New table + page that reads it** → `mixcore:db` → `mixcore:cms`
- **Build a landing page, marketing site, or any frontend page** → `mixcore:build-site` (Razor/CMS templates, all phases)
- **Only when user explicitly says "use mixcore:spa" or "I have a built dist folder"** → `frontend-design` (build the SPA) → `mixcore:spa` (install into Mixcore)
- **AI chat widget on a page** → `mixcore:ai` (hub/streaming/auth wiring) + `mixcore:cms` (widget HTML/CSS in the template content field)
- **Any coding task alongside CMS content** → `mixcore:dev` (for code) + `mixcore:cms` or `mixcore:db` (for content)

---

## Boundary Rules

- **`mixcore:cms` vs `mixcore:db`**: `mixcore:db` is schema-only. The moment a `.cshtml` template, page, or `Model.GetModule(...)` is involved, switch to `mixcore:cms`.
- **`mixcore:build-site` is the default for all website/landing page requests** — including React, Svelte, Vue, or "build a page" requests. It uses Razor/CMS templates and covers all phases automatically.
- **`mixcore:spa` is opt-in only**: use it exclusively when the user explicitly says "use mixcore:spa", "I have a dist folder", "embed a built SPA", or names `mixcore:spa` directly. Never infer it from the tech stack alone.
- **CMS vs coding**: If the task is about `.razor` components, C# services, `dotnet build`, or scaffolding a module — route to `mixcore:dev`, not here.
- **`mixcore:ai` vs `mixcore:cms`**: `mixcore:ai` owns the chat *behavior* (SignalR hub, `AskAI`, streaming handlers, auth/token). The widget's *HTML/CSS* (drawer, overlay, login form, bubbles) lives in the template content field and is `mixcore:cms`'s job. A typical widget task uses both.
...
| Wiki / RAG | `search` (discover), `read_document`, `generate_document`, `list_documents`, `delete_document`, `reload_wiki` — load **`mixcore:rag`** for all wiki doc ops |
