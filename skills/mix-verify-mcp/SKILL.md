...
Verifies tools over **raw JSON-RPC** (`tools/list` + `tools/call`) via curl against the running `/mcp` — not the `mcp__*` client. Smoke tool: `echo`. Target = whichever tool you changed.
<!-- /mcp-tools:auto -->
