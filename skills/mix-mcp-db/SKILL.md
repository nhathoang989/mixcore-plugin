...
- **Tables** — `create_mix_db_table`, `create_mix_db_table_from_prompt`, `get_mix_db_by_system_name`, `search_mix_db`, `update_mix_db_table`, `repair_mix_db_table`, `migrate_mix_db_table`, `delete_mix_db_table`
- **Columns** — `create_column`, `add_column_to_table`, `update_column`, `update_table_column`, `delete_column`, `delete_table_column`, `get_column_by_id`, `list_columns`, `get_columns_by_table`
- **Rows / data** — `query_table`, `query_rows`, `get_row_by_id`, `create_row`, `update_row`, `delete_row`, `create_many`, `update_many`
- **Relationships** — `create_relationship`, `get_relationship_by_id`, `list_relationships`, `update_relationship`, `delete_relationship`, `get_relationships_by_parent_table`, `get_relationships_by_child_table`
- **Query / raw DB** — `smart_query`, `parse_smart_query`, `execute_query`, `get_tables`, `get_table_schema`, `get_table_data`
<!-- /mcp-tools:auto -->
