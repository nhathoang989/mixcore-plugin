...
Drives pages with Playwright, then cross-checks the DB via these read tools (exact `tools/list` names).

- **Content / DB cross-check** — `list_page_contents`, `get_page_content_by_seo_name`, `list_templates`, `list_module_contents`, `query_table`, `execute_query`
- **AI-build audit** — `list_ai_plans`, `get_ai_plan`
<!-- /mcp-tools:auto -->
