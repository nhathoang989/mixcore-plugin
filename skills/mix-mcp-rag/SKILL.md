...
Canonical `/mcp` tools this skill owns (exact `tools/list` names; confirm signatures live via ToolSearch). Never use `read_text_file`/`write_text_file` on wiki docs — they bypass the RAG index.

- **Search** — `search`, `search_backend_knowledge`
