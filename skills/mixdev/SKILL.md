...
> **Scope:** this router covers tasks where the **AI edits repo source code to implement features** — C#, Blazor `.razor`, modules, EF migrations, dotnet CLI, xUnit tests. It does not CRUD CMS content. Sibling routers: **`mixcore:mixcore`** (content CRUD via MCP tools) · **`mixcore:docs-sync`** (CRUD developer docs, keep skills ↔ system-prompts consistent).

When invoked with a task (e.g. `/mixcore:mixdev add a Blazor sidebar` or `/mixcore:mixdev scaffold a payments module`), you **must**:
...
| Shadcn-style dashboard/UI shells from `BlazorBlueprint.Components` — layouts, data grids, charts, sidebars, forms, page blueprints | `mixcore:mix-dev-blazor-blueprint` |
| Blazor `.razor` components, pages, services, `HttpClient` wiring, render modes (`@rendermode`), routing, DI registration | `mixcore:mix-dev-blazor-app` |
| **Scaffold a module** — a brand-new standalone module/project from scratch (new csproj + `IStartupService` + sln registration) OR add a controller/service to an existing module | `mixcore:mix-dev-module` |
| C# 12 / .NET 10 code — **adding ViewModels, CQRS handlers, MCP tools, EF entities, or any non-controller/service class to an existing module**; `mix.heart` ViewModels, safe-default review | `mixcore:mix-dev-dotnet-code` |
...
- **New Blazor dashboard with live data** → `mixcore:mix-dev-blazor-blueprint` → `mixcore:mix-dev-blazor-app` (wire HttpClient + DI) → `mixcore:mix-dev-dotnet-code` (backend service) → **Playwright** (verify it renders + no console errors)
- **Scaffold a full feature module** → `mixcore:mix-dev-module` (skeleton + controller/service) → `mixcore:mix-dev-dotnet-code` (fill in logic) → `mixcore:mix-dev-dotnet-cli` (build + verify)
- **Backend bugfix + verify** → `mixcore:mix-dev-dotnet-code` → `mixcore:mix-dev-dotnet-cli` → `dotnet-test:run-tests` (run/filter the affected tests)
- **Tests for a new service** → `mixcore:mix-dev-dotnet-code` (review service) → `mixcore:mix-dev-tests` → `dotnet-test:code-testing-agent` (coverage) / `dotnet-test:test-gap-analysis` (find gaps)
- **Slow EF query / N+1** → `mixcore:mix-dev-dotnet-code` (locate query) → `dotnet-data:optimizing-ef-core-queries`
- **Slow / failing build** → `mixcore:mix-dev-dotnet-cli` → `dotnet-msbuild:build-perf-diagnostics` or `dotnet-msbuild:binlog-failure-analysis`
- **New Blazor page with backend service** → `mixcore:mix-dev-blazor-app` → `dotnet-blazor:fetch-and-send-data` (async lifecycle) → `mixcore:mix-dev-dotnet-code` → **Playwright** (verify UI/UX)
...
- **`mixcore:mix-dev-blazor-blueprint` vs `mixcore:mix-dev-blazor-app`**: `blueprint` = layout/grid/chart shells from `BlazorBlueprint.Components`. `app` = HttpClient, DI, routing, render modes. Complex dashboards start with `blueprint`, then `app` for wiring.
- **`mixcore:mix-dev-module` vs `mixcore:mix-dev-dotnet-code`**: `mixcore:mix-dev-module` owns the module *skeleton* (new csproj + `IStartupService` + sln entry) and its *controller/service* surface — for both brand-new projects and additions to an existing module. `mixcore:mix-dev-dotnet-code` fills in everything else (ViewModels, CQRS handlers, MCP tools, EF entities, general logic). Scaffold with `mixcore:mix-dev-module` first, then fill with `mixcore:mix-dev-dotnet-code`.
- **`mixcore:mix-dev-dotnet-code` vs `mixcore:mix-dev-dotnet-cli`**: `dotnet-code` writes/reviews C# source files. `mixcore:mix-dev-dotnet-cli` runs terminal commands (build, test, migrate). Use both — code first, CLI to verify.
- **`mixcore:mix-dev-tests` vs `mixcore:mix-dev-dotnet-cli`**: `mixcore:mix-dev-tests` generates test source files. `mixcore:mix-dev-dotnet-cli` runs them. Always pair.
- **Dev vs CMS**: If the task touches `.cshtml` Razor view templates, MixDB tables, or CMS pages/modules/posts — that's `mixcore:mixcore` (not `mixcore:mixdev`).
