...
```
# 1. Create the detail template (mixcore:cms territory; use @model dynamic + @inject IMixDbDataService)
...
Use `generate_document` (load **`mixcore:rag`**) with `folder: "<site-name>/database"` and `title: "<system_name>"` — it writes the file **and** indexes it atomically (no `reload_wiki` needed). Body template:
...
- After `delete_mix_db_table`, remove the corresponding wiki doc with `delete_document` (mixcore:rag) — it de-indexes automatically.
