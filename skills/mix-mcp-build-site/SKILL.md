...
- 🚨 **`ValidateTemplate` is compile-only — it does NOT catch unwrapped CSS/JS or stray `@section` blocks.** After it returns `success:true`, also eyeball the content you passed: every CSS block MUST be wrapped in `<style>…</style>` and every JS block in `<script>…</script>` — the `styles`/`scripts` fields AND inline body content are emitted **raw** (no auto-wrap), so a bare block prints as **literal text** on the page instead of styling/running. And the template must have **zero `@section` definitions** (page/post/module/widget/form render as partials → `@section` is silently dropped; masters `@RenderSection` only). Fix with `SearchReplaceTemplate`/`UpdateTemplate`. See `mixcore:mix-mcp-cms/references/razor-rules.md` §0.5–§0.6.
...
🚨 **Phase 3→4 handoff: module rendering in page templates.** When writing page templates in Phase 4 that iterate `Model.Modules`, each module's `TemplateFilePath` is already an absolute path — use `@await Html.PartialAsync(md.TemplateFilePath, md)` bare, never `"../Modules/" + md.TemplateFilePath`. See `mixcore:mix-mcp-cms/references/razor-rules.md` §7.
...
0b. **Razor-rules audit on ALL templates (run after the gates, before the browser pass)** — `validate_template`/`validate_site_sections` are compile/section checks and do **NOT** catch unwrapped CSS/JS or the `"../Modules/" + TemplateFilePath` bug. Call `ListTemplates` for the site theme, then `GetTemplate(id)` on each and check the `content`, `styles`, and `scripts`:
   - **Every CSS block wrapped in `<style>…</style>`, every JS block in `<script>…</script>`** — the `styles`/`scripts` fields and inline body are emitted raw, so a bare block prints as visible literal text (you'll see CSS rules / JS source in the page). The `styles` field MUST start with `<style>`/end with `</style>`; `scripts` with `<script>`/`</script>`.
   - **Zero `@section` definitions** in page/post/module/widget/form templates (silently dropped); masters `@RenderSection` only.
   - **No `"../Modules/" + md.TemplateFilePath` concatenation** — in page templates, every `@await Html.PartialAsync(...)` inside a `Model.Modules` loop must use `md.TemplateFilePath` bare. `TemplateFilePath` is already an absolute path; prefixing `"../Modules/"` produces a garbage doubled path → `InvalidOperationException: partial view '../Modules//wwwroot/…' was not found`. See `mixcore:mix-mcp-cms/references/razor-rules.md` §7.
   - Fix any violation with `SearchReplaceTemplate`/`UpdateTemplate` and re-check. See `mixcore:mix-mcp-cms/references/razor-rules.md` §0.5–§0.6.
...
5b. **Orphan cleanup** — initial builds often leave duplicate module content instances (suffixed systemNames like `hero-banner-module`) unassociated with any page. Cross-check `ListModules` against `ListPageModuleAssociations` and delete the leftovers — they pollute the system and confuse later audits.
5c. **Full runtime QA (recommended final sweep)** — the gates in task 0 are compile/section/query checks and the browser pass is spot coverage; neither catches every runtime JS error (scroll/animation handlers), dead link, or favicon 404. Run `mixcore:mix-verify-full-scan` (or the portal's **AI → Web Builder → "Full Test"**) before declaring Phase 7 complete.
...
This skill orchestrates the leaf skills; below are the tools each phase drives (full signatures live in the owning skill).

- **Schema — `mixcore:mix-mcp-db`** — `create_mix_db_table_from_prompt`, `create_mix_db_table`, `create_column`, `create_relationship`, `migrate_mix_db_table`, `create_row`, `create_many`
- **Templates — `mixcore:mix-mcp-cms`** — `create_template`, `validate_template`, `update_template`, `list_templates`
- **Content — `mixcore:mix-mcp-cms`** — `create_page_content`, `create_module_content`, `create_post_content`, `create_page_module_association`, `reorder_modules_on_page`
- **Multilingual — `mixcore:mix-mcp-cms`** — `create_culture`, `create_language`, `set_language_content`
- **Verify** — `validate_site_sections`, `validate_site_queries`
<!-- /mcp-tools:auto -->
