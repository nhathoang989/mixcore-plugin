...
0b. **Razor-rules audit on ALL templates (run after the gates, before the browser pass)** — `validate_template`/`validate_site_sections` are compile/section checks and do **NOT** catch unwrapped CSS/JS. Call `ListTemplates` for the site theme, then `GetTemplate(id)` on each and check the `content`, `styles`, and `scripts`:
