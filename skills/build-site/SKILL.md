...
> **Implementation tools**: Invoke `mixcore:cms` before any template/content/MCP work. Invoke `mixcore:db` before any schema/column/relationship work.
...
3. Invoke the relevant sub-skill (`mixcore:cms` or `mixcore:db`) for that phase's work
...
|---|---|---|---|
| 1 | Database & Theme Setup | `mixcore:db` | MixDB tables seeded, IDs recorded |
| 2 | Master Layout | `mixcore:cms` | Master template created, layoutId recorded |
| 3 | Module Templates & Content | `mixcore:cms` | Module templates + content instances, systemNames recorded |
| 4 | Main Pages | `mixcore:cms` | Home, About, Services, Contact pages created/updated |
| 5 | Secondary Pages | `mixcore:cms` | Blog listing, category, detail pages |
| 6 | Forms & Widgets | `mixcore:cms` + `mixcore:db` | Contact form, newsletter, sidebar widgets |
| 7 | Verify & Fix | `mixcore:cms` + browser | All pages render correctly, bugs fixed |
...
> See `mixcore:db` skill for `CreateMixDbTableFromPrompt` parameter format and relationship rules.
...
**Invoke `mixcore:cms` skill first.**
...
1. **Forms** — `CreateTemplate(folderType: "Forms")`:
   ...
   - **Numeric columns** (`Double`, `Integer`): the `frm-mixdb-ajax` handler must coerce `FormData` strings to JS numbers before `JSON.stringify`, or PostgreSQL raises error 42804 (`column "x" is of type double precision but expression is of type text`). Use `/^-?\d+(\.\d+)?$/.test(v) ? parseFloat(v) : v` in the `forEach` loop. See `mixcore:cms/references/form-templates.md` § Numeric fields.
...
**Invoke `mixcore:cms` skill first. Use Playwright browser tools if available.**
...
> use `generate_document` (load **`mixcore:rag`**) — it writes and indexes atomically.
...
- mixcore:db (phases 1, 6 if new tables needed)
- mixcore:cms (phases 2–7)
...
- For all other implementation rules, see `mixcore:cms` skill (Razor syntax, MixDB queries, module rendering)
