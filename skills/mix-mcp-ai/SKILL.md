...
This skill wires the browser chat widget to the **SiteKnowledge SignalR hub** (`AskAI` invocation + streaming events) — it does **not** call `/mcp` tools directly. The server-side agent that `AskAI` dispatches has the full tool catalog; see `mix-mcp-reference` for the complete list. No MCP tool belongs to this skill.
<!-- /mcp-tools:auto -->
